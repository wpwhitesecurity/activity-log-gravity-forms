...
if ( ! class_exists( '\WSAL\Plugin_Sensors\Gravity_Forms_Sensor' ) ) {
	/**
	...
	class Gravity_Forms_Sensor {
		...
		public static function event_form_meta_updated( $form_meta, $form_id, $meta_name ) {
			if ( isset( self::$_old_form ) ) {
				...
				$compare_changed_items = array_diff_assoc(
					...
					array_map( 'serialize', self::$_old_form )
				);
				...
				foreach ( $changed_data as $changed_setting => $value ) {
					if ( 'confirmations' === $changed_setting ) {
						$old_confirmations = self::$_old_form['confirmations'];
					...
					if ( 'notifications' === $changed_setting ) {
						$old_fields = self::$_old_form[ $changed_setting ];
						$alert_code = 5706;
					...
					if ( 'fields' === $changed_setting ) {
						...
						$old_fields     = self::$_old_form['fields'];
					...
					if ( 'personalData' === $changed_setting ) {
						...
						if ( isset( self::$_old_form[ $changed_setting ] ) ) {
							$old_fields = self::$_old_form[ $changed_setting ];
						} else {
							$old_fields = array();
						}
						...
						foreach ( $changed_items as $name => $value ) {
							...
							$event_type = 'modified';
							$old_value  = self::$_old_form[ $changed_setting ];
							...
							if ( ! Alert_Manager::was_triggered_recently( 5703 ) ) {
								$variables = array(
									...
									'old_setting_value' => ( isset( self::$_old_form[ $changed_setting ] ) && self::$_old_form[ $changed_setting ] ) ? $old_value : 'N/A',
		...
		public static function event_form_confirmation_saved( $confirmation, $form ) {
			...
			if ( isset( self::$_old_form['confirmations'] ) ) {
				foreach ( self::$_old_form['confirmations'] as $old_confirmation ) {
